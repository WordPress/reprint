...
writes one there) instead of the state directory. Deleting the state directory revokes the key: the public half
enrolled on the site is useless without the private half, and the plugin's key table has a Remove button for each
key. A site that only has a connection token answers `not_configured` (HTTP 503) on an OpenSSL host until a key is
enrolled.
