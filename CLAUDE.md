...
The `apply-runtime` command separates source host detection from target runtime configuration. The `remove-host-files` command uses the same host-analyzer manifest when an integration provides its own target runtime. The flow is:
