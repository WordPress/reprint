...
class DiagnoseHttpErrorTest extends TestCase
{
    ...
    public function testWordfenceThrottle503NamesTheFirewallAndNextSteps()
    {
        $body = '<!DOCTYPE html><html><head>' .
            '<title>Your access to this site has been limited</title>' .
            '</head><body><p>Generated by Wordfence</p></body></html>';

        $result = $this->diagnose(503, $body);

        $this->assertSame('WORDFENCE_BLOCKED', $result['code']);
        $this->assertStringContainsString('Wordfence', $result['message']);
        $this->assertStringContainsString('request limit', $result['message']);
        $this->assertStringContainsString('resume', $result['message']);
        $this->assertStringContainsString('allowlist', $result['message']);
        $this->assertStringNotContainsString('server crashed', $result['message']);
    }
    ...
    public function testPullJsonRequestReportsWordfenceBlockPageOnHttp200()
    {
        if (!function_exists('curl_init') || !function_exists('pcntl_fork')) {
            $this->markTestSkipped('Wordfence HTTP 200 wire coverage requires PHP curl and pcntl.');
        }
        ...
        $this->assertSame('WORDFENCE_BLOCKED', $result['error_code']);
        $this->assertStringContainsString('Wordfence', $result['error']);
        $this->assertStringContainsString('resume', $result['error']);
