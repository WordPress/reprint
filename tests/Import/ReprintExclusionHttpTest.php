...
class ReprintExclusionHttpTest extends TestCase
{
    ...
    public function testPullExcludesFilesAndDatabaseStateTogether(string $flag, bool $excluded): void
    {
        ...
        foreach (array_slice(array_column($source_options, 'option_name'), 0, 4) as $name) {
            ...
            $this->assertStringContainsString(base64_encode($name), $sql, 'Cleanup changes the destination, not db.sql.');
        }
    ...
    public function testSavedExclusionSurvivesSeparateCommands(): void
    {
        ...
        $sql = file_get_contents($this->root . '/state/db.sql');
        $this->assertStringContainsString(base64_encode('reprint_server_connection_token'), $sql);
        $this->run_client('db-apply', ['--target-engine=mysql', '--target-host=' . getenv('DB_HOST'),
            '--target-user=' . getenv('DB_USER'), '--target-pass=' . getenv('DB_PASS'), '--target-db=' . $this->target_database]);
        $this->assertSame(0, (int) $this->database->query("SELECT COUNT(*) FROM `{$this->target_database}`.custom_options WHERE option_name = 'reprint_server_connection_token'")->fetchColumn());
    ...
    public function testOnlyReprintApplyCleansTheDestinationAndPreservesTheDump(string $engine): void
    {
        ...
        $this->assertArrayHasKey('reprint_server_connection_token', $options);
        $this->assertSame(['renamed/index.php', 'other/index.php', 'renamed-extra/index.php'], unserialize($options['active_plugins']));
        $target->exec('COMMIT');
        $dump = file_get_contents($this->root . '/state/db.sql');
        $this->run_client('db-apply', ['--target-engine=mysql', '--target-host=' . getenv('DB_HOST'),
            '--target-user=' . getenv('DB_USER'), '--target-pass=' . getenv('DB_PASS'), '--target-db=' . $this->target_database]);
        $options = $target->query('SELECT option_name, option_value FROM custom_options')->fetchAll(PDO::FETCH_KEY_PAIR);
        $this->assertSame(['other/index.php', 'renamed-extra/index.php'], unserialize($options['active_plugins']));
        $this->assertSame(['active_plugins', 'another_plugin_token'], array_keys($options));
        $this->assertSame($dump, file_get_contents($this->root . '/state/db.sql'));
        $this->assertSame($source_options, $source->query('SELECT * FROM custom_options ORDER BY option_id')->fetchAll(PDO::FETCH_ASSOC));
    }

    public function testNetworkActivationRemainsInTheExportedSql(): void
    {
        foreach ([
        ...
        $this->assertSame(['renamed/index.php' => 12, 'other/index.php' => 34], unserialize($plugins));
        $this->assertSame($before, $this->database->query('SELECT * FROM custom_sitemeta ORDER BY meta_id')->fetchAll(PDO::FETCH_ASSOC));
    }

    /** SQLite applies perform the same cleanup without MySQL charset functions. */
    public function testSqliteTargetIsCleanedAfterImport(): void
    {
        $this->run_client('preflight');
        $this->run_client('db-pull', ['--exclude-reprint']);
        $this->run_client('db-apply', ['--target-engine=sqlite', '--target-sqlite-path=' . $this->root . '/target.sqlite']);
        $target = new PDO('sqlite:' . $this->root . '/target.sqlite');
        $options = $target->query('SELECT option_name, option_value FROM custom_options')->fetchAll(PDO::FETCH_KEY_PAIR);
        $this->assertArrayNotHasKey('reprint_server_connection_token', $options);
        $this->assertSame(['other/index.php', 'renamed-extra/index.php'], unserialize($options['active_plugins']));
    }

    /** Both import paths must finish cleanup before reporting completion. */
    public static function import_modes(): array
    {
        return [['file'], ['mysql']];
    }

    /**
     * A target user without DELETE can import SQL, but cannot remove credentials.
     * Granting that permission must allow cleanup to resume without replaying SQL.
     *
     * @dataProvider import_modes
     */
    public function testFailedCleanupResumesWithoutTheSourceOrReimportingSql(string $mode): void
    {
        $user = 'cleanup_' . bin2hex(random_bytes(5));
        $this->database->exec("CREATE USER '{$user}'@'%' IDENTIFIED BY 'test-password'");
        try {
            $this->database->exec("GRANT SELECT, INSERT, UPDATE, CREATE, DROP, ALTER, INDEX ON `{$this->target_database}`.* TO '{$user}'@'%'");
            $position_table = (new \ReflectionClass(\ImportClient::class))->getConstant('DATABASE_IMPORT_POSITION_TABLE');
            $this->database->exec("GRANT CREATE, DELETE ON `{$this->target_database}`.`{$position_table}` TO '{$user}'@'%'");
            $this->run_client('preflight');
            if ($mode === 'file') {
                $this->run_client('db-pull', ['--exclude-reprint']);
                $command = 'db-apply';
                $arguments = ['--target-engine=mysql', '--target-host=' . getenv('DB_HOST'),
                    '--target-user=' . $user, '--target-pass=test-password', '--target-db=' . $this->target_database];
            } else {
                $command = 'db-pull';
                $arguments = ['--exclude-reprint', '--sql-output=mysql', '--mysql-host=' . getenv('DB_HOST'),
                    '--mysql-user=' . $user, '--mysql-password=test-password', '--mysql-database=' . $this->target_database];
            }
            $this->run_client($command, $arguments, 1);
            $this->assertStringContainsString('DELETE command denied', file_get_contents($this->root . '/client.log'));
            $this->assertSame(str_repeat('a', 64), $this->database->query("SELECT option_value FROM `{$this->target_database}`.custom_options WHERE option_name = 'reprint_server_connection_token'")->fetchColumn());

            $this->database->exec("GRANT DELETE ON `{$this->target_database}`.* TO '{$user}'@'%'");
            // This is ordinary destination data edited after SQL import, not
            // private checkpoint state. Reimporting would overwrite the edit.
            $this->database->exec("UPDATE `{$this->target_database}`.custom_options SET option_value = 'edited-after-import' WHERE option_name = 'another_plugin_token'");
            proc_terminate($this->server);
            proc_close($this->server);
            $this->server = null;
            $this->run_client($command, $arguments);
            $options = $this->database->query("SELECT option_name, option_value FROM `{$this->target_database}`.custom_options")->fetchAll(PDO::FETCH_KEY_PAIR);
            $this->assertArrayNotHasKey('reprint_server_connection_token', $options);
            $this->assertSame('edited-after-import', $options['another_plugin_token']);
            $this->assertSame(['other/index.php', 'renamed-extra/index.php'], unserialize($options['active_plugins']));
        } finally {
            $this->database->exec("DROP USER '{$user}'@'%'");
        }
    }

    /**
     * Process death can occur before the first cleanup statement or after its commit.
     */
    public static function cleanup_boundaries(): array
    {
        return [['file', 'before-cleanup'], ['file', 'after-cleanup'], ['mysql', 'before-cleanup'], ['mysql', 'after-cleanup']];
    }

    /** @dataProvider cleanup_boundaries */
    public function testProcessDeathAtCleanupBoundaryResumesLocally(string $mode, string $boundary): void
    {
        if (!function_exists('posix_kill')) {
            $this->markTestSkipped('This process-death test requires posix_kill().');
        }
        $this->run_client('preflight');
        if ($mode === 'file') {
            $this->run_client('db-pull', ['--exclude-reprint']);
            $options = ['command' => 'db-apply', 'target_engine' => 'mysql', 'target_host' => getenv('DB_HOST'),
                'target_user' => getenv('DB_USER'), 'target_pass' => getenv('DB_PASS'), 'target_db' => $this->target_database];
        } else {
            $options = ['command' => 'db-pull', 'sql_output' => 'mysql', 'mysql_host' => getenv('DB_HOST'),
                'mysql_user' => getenv('DB_USER'), 'mysql_password' => getenv('DB_PASS'), 'mysql_database' => $this->target_database];
        }
        $options['exclude_reprint'] = true;
        $log = $this->root . '/interrupted-client.log';
        $process = proc_open([PHP_BINARY, __DIR__ . '/fixtures/reprint-cleanup-interruption.php',
            $this->url, $this->root, $boundary, json_encode($options)], [
            0 => ['pipe', 'r'], 1 => ['file', $log, 'w'], 2 => ['file', $log, 'a'],
        ], $pipes);
        fclose($pipes[0]);
        $this->assertNotSame(0, proc_close($process), file_get_contents($log));
        $this->assertFileExists($this->root . '/killed-at-cleanup', file_get_contents($log));
        $this->assertSame($boundary, file_get_contents($this->root . '/killed-at-cleanup'));
        $this->assertSame($boundary === 'before-cleanup' ? 1 : 0, (int) $this->database->query("SELECT COUNT(*) FROM `{$this->target_database}`.custom_options WHERE option_name = 'reprint_server_connection_token'")->fetchColumn());
        $this->database->exec("UPDATE `{$this->target_database}`.custom_options SET option_value = 'edited-after-import' WHERE option_name = 'another_plugin_token'");
        proc_terminate($this->server);
        proc_close($this->server);
        $this->server = null;
        ...
        $arguments = [];
        foreach ($options as $name => $value) {
            $arguments[] = '--' . str_replace('_', '-', $name) . ($value === true ? '' : '=' . $value);
        }
        $this->run_client($command, $arguments);
        $options = $this->database->query("SELECT option_name, option_value FROM `{$this->target_database}`.custom_options")->fetchAll(PDO::FETCH_KEY_PAIR);
        $this->assertArrayNotHasKey('reprint_server_connection_token', $options);
    ...
    /**
     * Run the real command and retain its output for failure assertions.
     *
     * @param string[] $arguments CLI arguments appended to the fixture paths.
     */
    private function run_client(string $command, array $arguments = [], int $expected_exit = 0): void
    {
        ...
        fclose($pipes[0]);
        $this->assertSame($expected_exit, proc_close($process), file_get_contents($log) . "\n" . file_get_contents($this->root . '/server.log'));
    }
}
