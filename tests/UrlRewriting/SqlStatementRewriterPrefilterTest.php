 ...
 * independent of how the leaf rewriter recognises literal URL boundaries.
 *
...
class SqlStatementRewriterPrefilterTest extends TestCase
{
     ...
     * URL boundary so the literal base splicer consistently recognises it.
    ...
    {
        // Padding ends in a space so the literal writer sees a start boundary.
    ...
    public function testLeavesEscapedJsonUrlUnchangedAtVariousAlignments(): void
    {
        $rewriter = $this->createRewriter();
        for ($pad_len = 0; $pad_len < 12; $pad_len++) {
            $padding = str_repeat('p', $pad_len) . ' ';
            ...
            $this->assertSame(
                $blob,
                $decoded,
                "pad_len={$pad_len} changed an escaped JSON URL"
            );
        }
    }
