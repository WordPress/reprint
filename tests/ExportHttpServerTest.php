...
final class ExportHttpServerTest extends TestCase
{
    ...
    public function testCursorAlwaysUsesCompressedTransportRepresentation(): void
    {
        $cursor_header = Site_Export_HTTP_Server::encode_cursor('{"offset":10}');
        $encoded_bytes = base64_decode($cursor_header, true);

        $this->assertIsString($encoded_bytes);
        $this->assertSame("\x1f\x8b", substr($encoded_bytes, 0, 2));
    }
