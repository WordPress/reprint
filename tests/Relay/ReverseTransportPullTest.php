 ...
 * entirely over the reversed single-endpoint channel: the importer (remote,
 ...
 * process (RelayExportSource). No curl, no sockets — just the relay seam.
 */
final class ReverseTransportPullTest extends TestCase
{
    ...
    public function testFilesPullMirrorsSourceOverTheReverseChannel(): void
    {
        ...
        $exchange = new \RelayExchange($driver);
        $worker   = new \RelaySourceWorker(
            static fn(array $request): array => $exchange->handle($request),
        ...
        fwrite(STDERR, sprintf(
            "\n--- reverse files-pull: %d files mirrored over %d outbound exchanges ---\n",
            count($sourceTree),
            count($transcript)
        ));
