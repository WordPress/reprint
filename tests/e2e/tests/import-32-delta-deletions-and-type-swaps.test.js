...
`);
    ...
    it('state stores path fields in base64 form', () => {
        ...
        assert.equal(typeof state.diff.last_consumed_remote_index_path, 'string', 'Expected diff.last_consumed_remote_index_path to be persisted');
        assert.ok(
            state.diff.last_consumed_remote_index_path.startsWith('base64:'),
            `Expected base64-encoded diff.last_consumed_remote_index_path, got: ${state.diff.last_consumed_remote_index_path}`,
        );
