...
describe('Import: --include + --remap onto a managed Atomic docroot', () => {
    ...
    // Shared remap and symlink flags; the --include scope varies per run.
    ...
    // ================================================================
    it('files-pull completes with --include + --remap and default local preservation', () => {
        const result = runImporter(importUrl(), tempDir, 'files-pull', {
            secret: getSiteSecret(site),
            extraArgs: ['--include', ':wp-content:', ...remap, ...symlinkOptions],
        });
        assert.equal(
            result.exitCode, 0,
    ...
    it('scoped delta re-sync completes (--include :wp-content:/plugins)', () => {
        const result = runImporter(importUrl(), tempDir, 'files-pull', {
            secret: getSiteSecret(site),
            extraArgs: ['--include', ':wp-content:/plugins', ...remap, ...symlinkOptions],
        });
        assert.equal(
            result.exitCode, 0,
