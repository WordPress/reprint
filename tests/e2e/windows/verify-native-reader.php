...
foreach (['backslash', 'forward-slash', 'root-relative', 'absolute-forward-slash'] as $reprint_spelling) {
    // PHP cannot read these stored root-relative targets; the native reader can.
    ...
    foreach (['directory', 'file'] as $reprint_kind) {
        ...
        if (!is_link($reprint_link_path) || !is_link(source_io_path($reprint_link_path))) {
        ...
        if ($reprint_spelling !== 'root-relative' && $reprint_ordinary_contents !== 'namespace file') {
