...
/**
 * Resolves all Windows source selections before the client builds its path rules.
 *
 ...
 *
 * @param array $config {
 *     @type string $source_paths_b64 JSON list of base64-encoded source paths.
 *                                   One form field avoids max_input_vars truncation.
 * }
 * @return array {
 *     @type bool     $ok        True after every path has resolved.
 *     @type string[] $paths_b64 Base64-encoded absolute paths, in request order.
 * }
 */
...
function endpoint_resolve_windows_paths(array $config): array {
    if (PHP_OS !== 'WINNT') {
        throw new InvalidArgumentException('Windows path resolution requires a Windows source.');
    }
    $encoded = $config['source_paths_b64'] ?? null;
    $paths = is_string($encoded) ? json_decode($encoded) : null;
    if (!is_array($paths) || $paths === []) {
        throw new InvalidArgumentException('source_paths_b64 must be a non-empty JSON list of base64-encoded paths; received ' . json_encode($encoded) . '.');
    }
    $directory = getcwd();
    if ($directory === false) {
    ...
    // Keep results private until every path succeeds. A rejected path fails the
    // whole selection; the client must never build rules from a partial list.
    $resolved_paths = [];
    foreach ($paths as $index => $encoded_path) {
        $path = is_string($encoded_path) ? base64_decode($encoded_path, true) : false;
        if ($path === false || $path === '' || strpos($path, "\0") !== false) {
            throw new InvalidArgumentException('source_paths_b64 entry ' . $index . ' must encode a non-empty Windows path without NUL bytes; received ' . json_encode($encoded_path) . '.');
        }
        ...
        $resolved_paths[] = base64_encode($resolved);
    }
    $response = ['ok' => true, 'paths_b64' => $resolved_paths];
    header('Content-Type: application/octet-stream');
