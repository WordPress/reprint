...
class PlaygroundCliApplier implements RuntimeApplier
{
    ...
    public function apply(RuntimeConfiguration $configuration, string $filesystem_root, string $output_dir, array $options = []): array
    {
        ...
        $saved_sqlite = $configuration->sqlite;
        $configuration->sqlite = null;
        ...
        $extra_mounts = $this->prepare_extra_mounts($configuration);
        ...
        $runtime = generate_runtime_php($configuration, self::VFS_ROOT);
        ...
        // Restore sqlite on the configuration so the caller can still report it.
        $configuration->sqlite = $saved_sqlite;
     ...
     * @return string[]
     */
    private function prepare_extra_mounts(RuntimeConfiguration $configuration): array
    {
        $mounts = [];
        $runtime_file_mounts = [
        ...
        foreach ($runtime_file_mounts as $constant_name => $vfs_path) {
            $host_path = $configuration->constants[$constant_name] ?? null;
            if (!is_string($host_path) || $host_path === '') {
                continue;
            }

            $configuration->constants[$constant_name] = $vfs_path;
    ...
    private function generate_start_script(
        ...
        $lines[] = '#';
        $lines[] = '# Source host: ' . $configuration->source;
        $lines[] = '';
