 ...
 */
function generate_runtime_php(RuntimeConfiguration $configuration, string $filesystem_root): string
{
    $lines = [];
    $lines[] = '<?php';
    $lines[] = '/**';
    ...
    if ($configuration->has_db_constants) {
    ...
    // Constants
    if (!empty($configuration->constants)) {
        foreach ($configuration->constants as $name => $value) {
    ...
    // Server variables
    if (!empty($configuration->server_vars)) {
        foreach ($configuration->server_vars as $name => $value) {
            $resolved = resolve_runtime_placeholders($value, $filesystem_root);
            $escaped = addslashes($resolved);
    ...
    if ($configuration->sqlite !== null) {
        $lines[] = generate_sqlite_loader_code($configuration->sqlite);
        $lines[] = '';
    }

    // Route handlers
    $route_code = generate_route_handler_code($configuration);
    if ($route_code !== '') {
        $lines[] = $route_code;
    }
...
/**
 * Generate PHP code for all route handlers declared in the configuration.
 *
 ...
 */
function generate_route_handler_code(RuntimeConfiguration $configuration): string
{
    $code = '';
