    ...
    private function execute_database_import_group(
        ...
        if ($target_engine === 'mysql') {
            ...
            while ($query_stream->next_query()) {
                ...
                // SHOW CREATE TABLE retains ENGINE=MyISAM, but a target can override
                // it with enforce_storage_engine. InnoDB cannot number rows within
                // each composite-key prefix; its auto-number column must lead an index.
                $create_table = null;
                $lexer = new \WP_MySQL_Lexer($query);
                if (
                    stripos($query, 'AUTO_INCREMENT') !== false &&
                    ...
                    if ($statement !== null) {
                        $create_table = $statement->get_first_descendant_node('createTable');
                    }
                }
                $engine = $create_table === null ? null : $create_table->get_first_descendant_node('engineRef');
                $elements = $create_table === null ? null : $create_table->get_first_child_node('tableElementList');
                if ($engine !== null && $elements !== null && strcasecmp($engine->get_first_descendant_token()->get_value(), 'MyISAM') === 0) {
                    ...
                    if (count($auto_increment_columns) === 1) {
                        ...
                        if (!$has_leading_index) {
                            $result = $connection->query("SHOW SESSION VARIABLES LIKE 'enforce_storage_engine'");
                            ...
                            if ($enforced_engine !== false && strcasecmp($enforced_engine['Value'], 'InnoDB') === 0) {
                                ...
                                $query = substr_replace($query, ', KEY (' . $column->get_bytes() . ')', $closing_parenthesis->start, 0);
                                $table_parts = [];
                                ...
                                $warning = "Warning: The target forces InnoDB. Adding a non-unique index on {$table_name}.{$column_name} " .
                                ...
                                $this->progress->clear_progress_line();
                                $this->progress->print_line($warning . "\n");
                            }
                        }
                    }
                }
