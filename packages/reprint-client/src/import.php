...
// Command-line parsing, validation, help, and application execution.
require_once __DIR__ . '/lib/cli/load.php';
...
if (
	...
	try {
		$reprint_exit_code                 = $reprint_application->run( $argv, ! $reprint_exit_after_pull );
		...
		if ( $reprint_exit_after_pull ) {
			// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- A CLI process exit status is not output.
			exit( $reprint_exit_code );
		}
		return;
