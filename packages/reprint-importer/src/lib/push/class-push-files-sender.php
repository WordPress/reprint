 ...
 * offset, the type, size, and ctime saved for a partial file, retry count, and
 * learned request limits needed to join those protocols after a process restart.
 *
 * ## Usage
 *
 ...
 * when its current type, size, and ctime equal the values saved after the prior
 * accepted part. Otherwise it starts that path at offset zero. This prevents
 * bytes from different local file versions from being joined.
 *
 ...
 * @phpstan-type SelectedLocalPath array{path:string,path_b64:string,local_paths_to_push_byte_offset:int,next_local_paths_to_push_byte_offset:int,local_path_change_fields:LocalPathChangeFields|null}
 * @phpstan-type State array{push_session_id:string,phase:'creating'|'planning'|'pushing_paths'|'pushing_deletes'|'committing'|'removing',local_paths_to_push_byte_offset:int,local_path_change_fields:LocalPathChangeFields|null,recoverable_failures:int,max_part_bytes:int|null,request_sizer_state:array{request_body_bytes:int,ceiling_bytes:int|null,growth_holdoff_remaining:int}}
 */
final class PushFilesSender
{
     ...
     *
     * @param array $options {
     *     Sender, transport, and local-file configuration.
     *
    ...
    private function next_local_path_upload_part(array &$state): array
    {
        ...
        try {
            ...
            $selected_local_path = $selected_path === null
                ? null
                : [
                    ...
                    'local_path_change_fields' => $this->read_local_path_change_fields($selected_path['path']),
                ];
        } catch (RuntimeException $exception) {
        ...
        fclose($local_paths_to_push_handle);
        if ($selected_local_path === null) {
            $state['local_path_change_fields'] = null;
            $state['phase'] = 'pushing_deletes';
            $this->store_state($state);
            return $this->step_result('continue', $state, null, null);
        }
        if ($selected_local_path['local_path_change_fields'] === null) {
            $state['phase'] = 'removing';
            $this->store_state($state);
            return $this->step_result('continue', $state, 'local_path_changed', 'A selected local path disappeared; remove the upload-only push session before generating another index.');
        }

        $request = $this->control_request('GET', 'push_status', [
            'push_session_id' => $state['push_session_id'],
            'path_b64' => $selected_local_path['path_b64'],
        ], ['accepted']);
        $failure = $this->handle_request_failure($request, $state);
        if ($failure !== null) {
            return $failure;
        ...
        $local_path_change_fields = $this->read_local_path_change_fields($selected_local_path['path']);
        ...
        if (
            ...
            && $path_type === $local_path_change_fields['type']
            && ( $local_path_change_fields['type'] !== 'file' || $path_status['accepted_bytes'] === $local_path_change_fields['size'] )
        ) {
        ...
        $confirmed_bytes = $saved_change_fields_match
            && $path_status['state'] === 'partial'
            && $path_type === 'file'
            && $path_status['accepted_bytes'] <= $local_path_change_fields['size']
        ...
        if ($local_path_change_fields['type'] === 'directory') {
            $directory_is_empty = $this->directory_is_empty($selected_local_path['path']);
            if ($directory_is_empty === null) {
                return $this->step_result('failed', $state, 'local_io_error', 'Could not read the selected local directory: ' . base64_encode($selected_local_path['path']) . '.');
            }
            $local_path_change_fields_after_read = $this->read_local_path_change_fields($selected_local_path['path']);
            ...
            if ($local_path_change_fields_after_read !== $local_path_change_fields) {
                $this->store_state($state);
                return $this->step_result('continue', $state, 'local_path_changed', 'The selected local path changed while it was being read; retry it from receiver-confirmed state.');
            }
            ...
            $upload_part = [
                'type' => 'directory',
                'path' => $selected_local_path['path'],
                'payload' => '',
            ];
            $logical_value_complete = true;
        } elseif ($local_path_change_fields['type'] === 'symlink') {
            $symlink_target = @readlink($this->docroot . '/' . $selected_local_path['path']);
            $local_path_change_fields_after_read = $this->read_local_path_change_fields($selected_local_path['path']);
            if ($local_path_change_fields_after_read === null) {
                $state['phase'] = 'removing';
                $this->store_state($state);
            ...
            if ($symlink_target === false) {
                return $this->step_result('failed', $state, 'local_io_error', 'Could not read the selected local symlink target: ' . base64_encode($selected_local_path['path']) . '.');
            }
            $upload_part = [
                'type' => 'symlink',
                'path' => $selected_local_path['path'],
        ...
        if ($local_path_change_fields['type'] === 'file') {
            $file_byte_offset = $confirmed_bytes <= $local_path_change_fields['size']
                ? $confirmed_bytes
                : 0;
            ...
            if ($maximum_payload_bytes === 0) {
                $request_size_failure_detail = 'The current request-body budget cannot fit one MIME part for path ' . base64_encode($selected_local_path['path']) . '.';
            } else {
                $payload = '';
                if ($local_path_change_fields['size'] > 0) {
                    $file_handle = fopen($this->docroot . '/' . $selected_local_path['path'], 'rb');
                    if (!is_resource($file_handle) || fseek($file_handle, $file_byte_offset) !== 0) {
                        ...
                        if ($local_path_change_fields_after_read === null) {
                            ...
                            $local_failure_detail = 'Could not open the selected local file at its receiver-confirmed cursor: ' . base64_encode($selected_local_path['path']) . '.';
                        }
                    } else {
                        $payload = fread($file_handle, $maximum_payload_bytes);
                        fclose($file_handle);
                        $local_path_change_fields_after_read = $this->read_local_path_change_fields($selected_local_path['path']);
                        if ($local_path_change_fields_after_read === null) {
                            $local_path_disappeared = true;
                            ...
                            $local_failure_detail = 'Could not read the selected local file at its receiver-confirmed cursor: ' . base64_encode($selected_local_path['path']) . '.';
                        } else {
                            $upload_part = [
                                'type' => 'file',
                                'path' => $selected_local_path['path'],
                            ...
                            $logical_value_complete = $file_byte_offset + strlen($payload) === $local_path_change_fields['size'];
                        }
                    }
                } else {
                    $upload_part = [
        ...
        if ($logical_value_complete) {
            ...
            $state['local_paths_to_push_byte_offset'] = $selected_local_path['local_paths_to_push_byte_offset'];
    ...
    /**
     * Reports whether a selected local directory has no child entry.
     *
    ...
    /**
     * Reads the fields used to decide whether a selected local path changed.
     *
