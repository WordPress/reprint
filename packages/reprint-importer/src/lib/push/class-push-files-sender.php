 ...
 * push, and saves the completed fresh local index as the local index
 ...
 * the local index for this remote Reprint API URL, one bounded step
 * at a time. A completed pull or target-committed push writes that index, and
 * files-diff also reads it. After planning
 ...
 * enters another phase which saves the fresh local index as the local index for
 * this remote Reprint API URL through a swap file. Index completion,
 ...
 * Reprint API URL remains beside sender.json. Once the plan result
...
final class PushFilesSender
{
    ...
    /** @var string Local index for this remote Reprint API URL. */
    ...
    private function __construct(array $options, ReprintProcessLock $process_lock)
    {
        ...
        $this->local_index_file = $this->push_state_directory . '/local_index.jsonl';
