...
// Shared file chunk reader/writer used by pull responses and push request bodies.
foreach ([
    'Site_Export_File_Chunk_Stream_Reader' => 'class-file-chunk-stream-reader.php',
    'Site_Export_File_Chunk_Stream_Writer' => 'class-file-chunk-stream-writer.php',
] as $file_chunk_stream_class => $file_chunk_stream_file) {
    if (class_exists($file_chunk_stream_class, false)) {
        continue;
    }
    foreach ([
        __DIR__ . '/../../reprint-exporter/src/' . $file_chunk_stream_file,
        __DIR__ . '/../../../vendor/wp-php-toolkit/reprint-exporter/src/' . $file_chunk_stream_file,
    ] as $file_chunk_stream_path) {
        if (file_exists($file_chunk_stream_path)) {
            require_once $file_chunk_stream_path;
            break;
        }
    }
}
        ...
        curl_setopt_array($ch, [
            ...
            CURLOPT_HEADERFUNCTION => function ($ch, $header_line) use (
                ...
                if (stripos($header_line, "Content-Type:") === 0) {
                    $boundary_value = Site_Export_File_Chunk_Stream_Reader::boundary_from_content_type($header_line);
                    if ($boundary_value !== null) {
                        $this->audit_log(
                            "Creating multipart parser with boundary: $boundary_value",
                            false,
                        );
                        $parser = new Site_Export_File_Chunk_Stream_Reader(
                            $boundary_value,
